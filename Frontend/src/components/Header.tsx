...
function Header() {
  ...
  return (
    <header className="max-w-[1280px] w-full mx-auto">
      {/* Top Contact Bar */}
      ...
      <nav className="w-full bg-white shadow-md relative">
        <div className="px-1 md:px-8 flex items-center justify-between py-2">
          {/* Logo */}
          ...
          {/* Desktop Menu */}
          <ul className="hidden lg:flex items-center gap-8 text-[0.95rem] font-medium text-gray-700">
            {/* All Courses with Dropdown */}
            <li className="relative">
              <button
                onClick={() => setCoursesOpen(!coursesOpen)}
                className={`flex items-center gap-1 hover:text-[#FA8128] transition-colors duration-200 py-2 ${coursesOpen ? "text-[#FA8128]" : ""}`}
              >
                All Courses
            ...
            {navItems.map((item) => (
              <li key={item.name}>
                <Link
                  to={item.path}
                  className="hover:text-[#FA8128] transition-colors duration-200 py-2"
